...
while IFS= read -r IP; do
  ...
  echo "正在启动服务器: $IP，实验 ID: $EXP_ID"

  # 通过 SSH 执行启动命令
  ssh -o StrictHostKeyChecking=no "$IP" \
    "docker exec $CONTAINER_NAME sh -c 'cd /collab && nohup ./run.sh $EXP_ID > /dev/null 2>&1 &'" > /dev/null 2>&1
  ...
  echo "✅ 服务器已在 $IP 上启动，实验 ID: $EXP_ID"
