...
type Config struct {
	...
	HotstuffTxSizePerBlock int             `json:"hotstuffTxSizePerBlock"` //当纯运行hotstuff的时候，每个区块包含多少笔交易
	Slow                   int             `json:"slow"`
	...
	BroadcastBySample      bool            `json:"broadcastBySample"`
...
//消息队列配置文件
type Mconfig struct {
	Address []string //消息队列的地址 ip+port
	Topic   string   //消息队列的主题 topic
	Enable  bool     //是否开启消息队列
}
